...
Vue.createApp({
  ...
  methods: {
    getStickies() {
      fetch(`http://aspapi.d.lymim.com/sticky`)
        .then(res => res.json())
        .then(objs => {
          objs.forEach(obj => {
            var sticky = {
    ...
    confirmEdit() {
      ...
      if (this.editingSticky.uid != null) {
        ...
        fetch(`http://aspapi.d.lymim.com/sticky?uid=${this.editingSticky.uid}`, {
          method: 'PUT',
          headers: {
    ...
    removeSticky(sticky) {
      ...
      fetch(`http://aspapi.d.lymim.com/sticky?uid=${sticky.uid}`, {
        method: 'DELETE',
      });
    },
